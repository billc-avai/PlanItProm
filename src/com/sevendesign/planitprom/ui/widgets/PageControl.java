...
public class PageControl extends LinearLayout {
    ...
    public PageControl(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        // now will wait until onFinishInflate to call initPageControl()
    }
    ...
    private void initPageControl() {
        ...
        activeDrawable.setAlpha(0);
        inactiveDrawable.setAlpha(0);

        mItemsCountText = new TextView(getContext());

        /* set up font */
        //KMFontManager manager = KMFontManager.getInstance();
        //mItemsCountText.setTypeface(manager.getTypeface(AppDefinitions.REGULAR));
    ...
    public void setActiveDrawable(Drawable d) {
        if (mPageCount <= mMaxDotsCount) {
            activeDrawable = d;
            if (android.os.Build.VERSION.SDK_INT >= 16) {
                indicators.get(mCurrentPage).setBackground(activeDrawable);
            } else {
    ...
    public void setInactiveDrawable(Drawable d) {
        if (mPageCount <= mMaxDotsCount) {
            ...
            for (int i = 0; i < mPageCount; i++) {
                if (android.os.Build.VERSION.SDK_INT >= 16) {
                    indicators.get(i).setBackground(inactiveDrawable);
                } else {
    ...
    public void setPageCount(int pageCount) {
        ...
        if (pageCount <= mMaxDotsCount) { // FOR DOTS
    ...
    public void setCurrentPage(int currentPage) {
        if (mPageCount <= mMaxDotsCount) { // DOTS FOR SMALL ITEMS AMOUNT
            if (currentPage < mPageCount) {
                if (android.os.Build.VERSION.SDK_INT >= 16) {
                    indicators.get(mCurrentPage).setBackground(inactiveDrawable);//reset old indicator
                    indicators.get(currentPage).setBackground(activeDrawable);//set up new indicator
                } else {
        ...
        } else { // TEXT FOR BIG ITEMS AMOUNT
            if (currentPage < mPageCount) {
                mItemsCountText.setText((currentPage + 1) + "/" + mPageCount);
                mCurrentPage = currentPage;
            }
        }
    }
